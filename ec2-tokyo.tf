...
resource "aws_security_group" "ec2-tokyo-sg443" {
  vpc_id = aws_vpc.TOKYO_VPC.id 
  /*
  tags = {
    Name = "tokyo-sg"
...
/*
#latest image  
data "aws_ami" "latest-amazon-linux-image-tokyo" {
    most_recent = true
    owners = ["amazon"]
    filter {
        name = "name"
...
resource "aws_instance" "ec2-tokyo-80" {
  # ami = data.aws_ami.latest-amazon-linux-image-tokyo.id
...
# Launch the EC2 instance for Port 443
resource "aws_instance" "ec2-tokyo-443" {
  #ami = data.aws_ami.latest-amazon-linux-image-tokyo.id
  ami = "ami-023ff3d4ab11b2525"
 ...
 user_data = base64encode(<<-EOF
  ...
  # Associate the instance with a security group
  vpc_security_group_ids = [aws_security_group.ec2-tokyo-sg443.id]
