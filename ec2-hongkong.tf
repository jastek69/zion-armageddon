...
resource "aws_security_group" "ec2-hongkong-sg80" {
...
/*
#latest image  
data "aws_ami" "latest-amazon-linux-image-hk" {
    most_recent = true
    owners = ["amazon"]
    filter {
        name = "name"
...
resource "aws_instance" "ec2-hongkong-80" {
  provider = aws.hongkong
  # ami = data.aws_ami.latest-amazon-linux-image-hk.id
  ami = "ami-08d17dd56120aa5b4"
 ...
 user_data = base64encode(<<-EOF
    ...
    <body>
    <div>
    <h1>Balactus has Arrived in Hong Kong</h1>
  ...
  vpc_security_group_ids = [aws_security_group.ec2-hongkong-sg80.id]

  # Associate the instance with a subnet
  subnet_id = aws_subnet.HK_SUBNET.id
