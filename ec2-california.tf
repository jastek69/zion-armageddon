...
resource "aws_instance" "ec2-cali-80" {
  provider = aws.california
  # ami = data.aws_ami.latest-amazon-linux-image-cali.id
 ...
 user_data = base64encode(<<-EOF
    ...
    <body>
    <div>
    <h1>Balactus has Arrived in California</h1>
  ...
  subnet_id = aws_subnet.CA_SUBNET.id
